...
class NeuralNetwork(nn.Module):
    ...
    def process(self, data, is_train):
        ...
        with torch.set_grad_enabled(is_train):
            for batch, (x, y) in enumerate(data):
                ...
                fully_correct += torch.logical_and( torch.logical_and(corrects[:,0], corrects[:,1]), corrects[:,2]).sum()

                if is_train and batch % 50 == 0:
                    print(f"loss:{loss.item():>3f} [{batch:>4d}/{data_size:>4d}] {100*batch/data_size:.1f}%")
        
        return loss_sum/data_size, part_correct/(len(data.dataset)*nb_elem_per_x), fully_correct/(len(data.dataset)*nb_elem_per_x)
    ...
    def debug_frame(self, x, y=None, name=""):
        ...
        for i in range(5):
            for j in range(5):
                #if y != None : print(y[i])
                cv2.imwrite('output/image_'+name+str(i)+str(j)+".png", x[i][j].cpu().detach().numpy()*255)
