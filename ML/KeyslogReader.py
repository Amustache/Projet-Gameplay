...
class KeyslogReader():
    def __init__(self, keys_path, frame_step=1, start_frame=0, offset=0):
        self.data = []
        ...
        for i in range(len(csv_file)):
            ...
            while current_frame < frame+offset:
                self.data.append(self.keysToTensor(current_state))
    ...
    def keysToTensor(self, keys) :
        output = torch.zeros(3)
        if "Key.left"  in keys : output[0] = 1
        if "Key.right" in keys : output[1] = 1
        if "'x'"       in keys : output[2] = 1
        return output
